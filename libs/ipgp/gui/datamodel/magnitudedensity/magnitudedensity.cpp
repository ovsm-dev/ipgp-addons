...
#include <ipgp/gui/datamodel/magnitudedensity/ui_magnitudedensity.h>
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
MagnitudeDensity::
MagnitudeDensity(DatabaseQuery* query, QWidget* parent, Qt::WFlags f) :
		PlottingWidget(query, parent, f), _ui(new Ui::MagnitudeDensity) {
	...
	setObjectName("MagnitudeDensity");
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
MagnitudeDensity::~MagnitudeDensity() {
	delete _ui, _ui = NULL;
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void MagnitudeDensity::print(const ToolBox::ExportConfig& ec) {
...
void MagnitudeDensity::replot() {
	...
	if ( t == eLatitude || t == eLongitude ) {
		_plot->xAxis->setLabel(QString(::STypes[_ui->comboBoxType->currentIndex()]));
		_plot->yAxis->setLabel(QString("Magnitude"));
	}
	else {
		_plot->yAxis->setLabel(QString(::STypes[_ui->comboBoxType->currentIndex()]));
		_plot->xAxis->setLabel(QString("Magnitude"));
	}
	...
	for (ItemList::const_iterator it = items.constBegin();
		...
		if ( t == ::eLatitude )
			g->addData((*it).latitude, (*it).magnitude);
		else if ( t == ::eLongitude )
			g->addData((*it).longitude, (*it).magnitude);
		else if ( t == ::eDepth )
		    g->addData((*it).magnitude, (*it).depth);
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void MagnitudeDensity::plottableClicked(QCPAbstractPlottable* plottable,
                                        QMouseEvent* event) {
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void MagnitudeDensity::showFormulaBox() {
	_ui->frameItemSize->hide();
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void MagnitudeDensity::showCustomSizeBox() {
