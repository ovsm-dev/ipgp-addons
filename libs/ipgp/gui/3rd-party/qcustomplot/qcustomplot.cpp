...
/**
 * @brief  This widget has been modified and received tweaks to fulfill some
 *         customs required features. Those changes are referenced by the use
 *         of a pattern (//! @author ...) so they can be easily spotted.
...
void QCPScatterStyle::drawShape(QCPPainter *painter, double x, double y) const
{
  ...
  switch (mShape)
  {
    ...
    case ssFilledSquare:
    {
      painter->fillRect(QRectF(x-w, y-w, mSize, mSize), painter->pen().color());
      break;
    }
    case ssDiamond:
    {
...
void QCPAxis::setupTickVectors()
{
  ...
  if (mAutoTickLabels)
  {
    ...
    //! @brief Sets the tickLabels to be of a custom pattern in which the first
	//!        tick on the left if displayed with full date time and the others
	//!        just display the time
	//! @note  The user has to set the DataTimeFormat to hh:mm:ss or hh:mm:ss.zzz
	//! @todo  Force the axis to be over-resized so it actually doesn't get
	//!        shorten when axis rectangle are piled up...
	else if (mTickLabelType == ltDateTimeAuto)
	{
	  for (int i=mLowestVisibleTick; i<=mHighestVisibleTick; ++i)
	  {
#if QT_VERSION < QT_VERSION_CHECK(4, 7, 0) // use fromMSecsSinceEpoch function if available, to gain sub-second accuracy on tick labels (e.g. for format "hh:mm:ss:zzz")
		if (i == mLowestVisibleTick)
		  mTickVectorLabels[i] = mParentPlot->locale().toString(QDateTime::fromTime_t(mTickVector.at(i)).toTimeSpec(mDateTimeSpec), QString("%1\nyyyy-MM-dd").arg(mDateTimeFormat));
		else
			mTickVectorLabels[i] = mParentPlot->locale().toString(QDateTime::fromTime_t(mTickVector.at(i)).toTimeSpec(mDateTimeSpec), mDateTimeFormat);
#else
		if (i == mLowestVisibleTick)
		{
			...
			.toTimeSpec(mDateTimeSpec), QString("%1\nyyyy-MM-dd").arg(mDateTimeFormat));
		}
		else
		{
			mTickVectorLabels[i] = mParentPlot->locale().toString(
				QDateTime::fromMSecsSinceEpoch(mTickVector.at(i)*1000)
			.toTimeSpec(mDateTimeSpec), mDateTimeFormat);
		}
#endif
	  }
	}
  ...
  mReplotting(false)
{
  ...
  //! @author Stephen Roselia
  //! Allows the widget to be painted with no background brush from the
  //! beginning. Default painter will use Qt::black even when Qt::transparent
  //! is specified, such behavior can be quite annoying...
...
//! @author Stephen Roselia
//! @note   Don't know why they removed this...
bool QCustomPlot::event(QEvent* event) {

	//! TODO Enhance the way tooltips are displayed in an interactive way :
	//! do not show only the tooltip but highlight it by making it visible
	//! in the same manner as graphs, items are displayed when selected
	//! (blue shadow) or even better, draw a customizable pen effect
	//! around the selected drawable...
    if (event->type() == QEvent::ToolTip) {
	  ...
	  if ( ret != -1 ) {
		  if (!mGraphs.at(ret)->tooltip().isEmpty()) {
			  mGraphs.at(ret)->setSelected(true);
			  QToolTip::showText(helpEvent->globalPos(), mGraphs.at(ret)->tooltip());
		  }
		  else {
			  mGraphs.at(ret)->setSelected(false);
		  }
      }
...
//! @author Stephen Roselia
//! @note   Don't know why they removed this...
const int QCustomPlot::graphAt(const QPoint& pos, bool onlySelectable) const {
...
//		graph->pixelsToCoords(pos.x(), pos.y(), x, y);
//		for (int k=0 ; k < graph->data()->values().size(); k++) {
//			if ( ((y-margin) < graph->data()->values().at(k).value)
//				  && (graph->data()->values().at(k).value < (y+margin))
//				  && ((x-margin) < graph->data()->values().at(k).key)
//				  && (graph->data()->values().at(k).key < (x+margin)) ) {
//				retCode = i;
//			}
//		}
//	}
